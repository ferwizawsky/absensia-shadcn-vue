...
const props = defineProps(["modelValue", "disabled", "data"]);
...
let filteredData = computed(() => {
  let filteredResults =
    query.value === ""
      ? option.listKelas
      : option.listKelas.filter((person) =>
          person.title
            .toLowerCase()
            .replace(/\s+/g, "")
...
watch(
  () => props.modelValue,
  (e) => {
    selected.value = option.listKelas.find(
      (x) => x.id === Number(props.modelValue)
    );
  },
  {
    deep: true,
        ...
        >
          ...
          >
            <li
              ...
              :class="{
                'bg-primary/40 title-white': active,
                '': !active,
              }"
            >
              <span
